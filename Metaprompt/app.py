...
metaprompt = '''Today you will be writing Chinese instructions to an eager, helpful, but inexperienced and unworldly AI assistant who needs careful instruction and examples to understand how best to behave. I will explain a task to you. You will write instructions that will direct the assistant on how best to accomplish the task consistently, accurately, and correctly. Here are some examples of tasks and instructions.
...
st.set_page_config(
        page_title="GenPrompt",
)

st.title('Claude3 GenPrompt')
with st.sidebar.subheader("选择模型"):
    model_choice = st.sidebar.selectbox("选择模型", ["anthropic.claude-3-sonnet-20240229-v1:0", "anthropic.claude-3-haiku-20240307-v1:0"])
    temperature = st.sidebar.slider("Temperature", 0.0, 4096.0, 0.5, 1.0)
    top_k = st.sidebar.slider("Top K", 0, 500, 250, 1)
    top_p = st.sidebar.slider("Top P", 0.0, 1.0, 1.0, 0.1)
#    stop_sequences = st.sidebar.text_area("Stop Sequences", "")
    
with st.sidebar.header("上传图片或者提供图片 URL"):
    uploaded_file = st.sidebar.file_uploader("上传图片",
                                                 type=['jpg', 'jpeg', 'png', 'gif', 'webp'],
                                                 on_change=clear_chat_history_message)
    
    
    image_URL = st.text_input("请输入图片地址")
...
def get_claude_prompt(prompt, assistant_partial):
    prompt_config = {
...
def gen_messages(input_text):
    content_image = None
    if uploaded_file:
        st.sidebar.image(uploaded_file)
        content_image = base64.b64encode(uploaded_file.read()).decode('utf8')
    elif image_URL:
        content_image = get_pic_encoding(image_URL)
    if input_text:
        show_chat_history()
...
def task_input(TASK):
    ...
    return (prompt)
def variable_input(VARIABLES):
    variable_string = ""
    assistant_partial = "<Inputs>"
    if VARIABLES:
        string_io = StringIO(VARIABLES)
        reader = csv.reader(string_io, delimiter=',', skipinitialspace=True)
        input_items = next(reader)
...
tab1,tab2 = st.tabs(["生成提示词模板","对话测试"])
with tab1:
    st.header("生成提示词模板")
    Task_input = st.text_area('使用自然语言输入需要处理的任务👇', "你是一名人工客服,需要根据客户的抱怨给出适当的回复")
    Variables_input = st.text_input('输入需要处理的变量,以 , 分割👇', '')
    if st.button("提交",on_click=clear_chat_history_message):
        
        with st.spinner('稍等片刻... ...'):
            #st.write(variable_input(Variables_input))
            final_prompt= get_claude_prompt(task_input(Task_input),variable_input(Variables_input))
            extracted_prompt_template = extract_prompt(final_prompt)
            variables = extract_variables(final_prompt)
            st.session_state.pe = extracted_prompt_template
            st.session_state.var = variables
            st.write(extracted_prompt_template)
            #st.write(variables)
            
with tab2:
    col1, col2 = st.columns(2)
    st.header("对话测试")
    prompt_on = st.toggle('请输入prompt中需要提供的数值')
    if 'var' in st.session_state:
        variables = st.session_state.var
    if prompt_on:
        if 'pe' in st.session_state:               
            systemprompt = st.session_state.pe
            #st.write(systemprompt)
            variable_values = {}
            user_inputs = {}
            for variable in variables:
                user_input = st.text_input(f"请输入 {variable} 的值:", key=variable)
                user_inputs[variable] = user_input
        else :
            st.write("缓存中没有 prompt")
    else : 
        systemprompt = ''
    variable_on = st.toggle('装载变量并修改提示词')
    if variable_on:
        for variable,value in user_inputs.items():
            systemprompt = systemprompt.replace("{" + variable + "}",value)
        st.write(systemprompt)
        #st.write (input_text)
    if st.button("加载提示词并测试"):
        if st.button:
            messages = gen_messages(systemprompt)
            #st.write (messages)
    ...
    if input_text:
        messages = gen_messages(input_text)
        #st.write (messages)
        with st.spinner('I am thinking about this...'):
            response = call_claude_sonnet(messages)
            #st.write(response)
